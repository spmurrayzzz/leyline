export function projectTranscriptEntries(entries, options = {}) {
  const toolCalls = collectToolCalls(entries)
  return entries.map((entry) => {
    return projectEntry(entry, toolCalls, options)
  }).filter(Boolean)
}
...
export function projectEntry(
  entry,
  toolCalls = collectToolCalls([entry]),
  options = {},
) {
  if (entry.type === 'message') {
    return projectMessageEntry(entry, toolCalls, options)
  }
...
function projectMessageEntry(entry, toolCalls, options = {}) {
  ...
  if (message.role === 'toolResult') {
    ...
    if (message.toolName === 'subagent') {
      ...
      if (isResearchSubagentDetails(projected.subagentDetails)) {
        projected.label = 'Research threads'
        projected.code = `${projected.subagentDetails.results.length} threads`
        projected.researchThreads = true
      }
    }
  ...
  if (entry.id === options.research?.reportEntryId) {
    projected.label = 'Assistant · research artifact'
    projected.researchReport = {
      title: options.research.reportTitle || 'Research report',
      sourceIds: options.research.citedSourceIds || [],
      sourceCount: options.research.sourceCount || 0,
      citedSourceCount: options.research.citedSourceCount || 0,
      sources: (options.research.sources || []).filter((source) => {
        return (options.research.citedSourceIds || []).includes(source.id)
      }).map((source) => ({
        id: source.id,
        key: source.key,
        url: source.url,
...
function skillCommandParts(text) {
  const prompt = parseSkillPrompt(text)
  if (prompt) {
    return { name: prompt.name, userMessage: prompt.userMessage }
  }
  if (typeof text !== 'string') return undefined
...
export function skillCommandText(text) {
  const skill = skillCommandParts(text)
  if (!skill) return ''
  return `/skill:${skill.name}${skill.userMessage ? ` ${skill.userMessage}` : ''}`
}

export function skillPromptDisplayText(text) {
...
export function toolAnnotation(toolName, call) {
  const args = toolCallArgs(call)

  if (toolName === 'read-skill') {
    const skill = args.path ? skillNameFromPath(args.path) : ''
    return {
      label: skill ? `Skill · ${skill}` : 'Skill',
      code: args.path ? shortPath(args.path) : undefined,
    }
  }
  ...
  if (toolName === 'bash') {
    return { label: 'Bash', code: args.command }
  }
  ...
  if (toolName === 'search_memory') {
    return { label: 'Memory · Search', code: args.query || '' }
  }
...
function isResearchSubagentDetails(details) {
  return Boolean(details?.results?.length)
    && details.results.every((result) => {
      return result.agent === 'researcher' || result.research
    })
}
