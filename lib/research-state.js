...
export function applyResearchEvent(state, event) {
  ...
  const startsCycle = event.kind === 'plan'
  const recoversReport = state.status === 'error' && event.kind === 'report'
  const repairsReport = state.status === 'error'
    && state.phase === 'report'
    && (event.kind === 'source'
      || (event.kind === 'phase' && event.phase === 'report'))
  if (state.status !== 'running'
    && !startsCycle
    && !recoversReport
    && !repairsReport) return state
  const restartsCycle = state.status !== 'running' && startsCycle
  const next = {
    ...state,
  ...
  if (event.kind === 'plan') {
    ...
    if (restartsCycle) {
      ...
      next.reportTitle = ''
      next.reportEntryId = ''
      next.reportRequestedAt = 0
      next.citedSourceIds = []
      next.error = ''
      next.errorCode = ''
      next.invalidLinks = []
      next.invalidLinkCount = 0
      next.matchedCitationCount = 0
      next.completedAt = 0
    }
  }
  ...
  if (event.kind === 'error') {
    ...
    next.errorCode = cleanText(event.code, 80)
      || (Array.isArray(event.invalidLinks)
        || Number.isFinite(event.matchedCount)
        ? 'citation_validation'
        : '')
    next.invalidLinks = normalizeInvalidLinks(event.invalidLinks)
    next.invalidLinkCount = Number.isFinite(event.invalidCount)
      ? Math.max(0, Number(event.invalidCount))
      : next.invalidLinks.length
    next.matchedCitationCount = Number.isFinite(event.matchedCount)
      ? Math.max(0, Number(event.matchedCount))
      : 0
  }
...
export function compactResearchState(state) {
  ...
  return {
    ...
    invalidLinkCount: state.invalidLinkCount || state.invalidLinks?.length || 0,
    updatedAt: state.updatedAt,
  }
}

export function canonicalResearchSourceKey(source, cwd = '') {
  ...
  const value = cleanText(source?.path, 2000)
  const path = cwd
    ? resolveResearchSourcePath(value, cwd)
    : value.replace(/^\.\//, '')
  if (path) return `path:${path}`
  return ''
}

export function researchSourcesMatch(left, right, cwd = '') {
  const leftKeys = new Set(researchSourceKeys(left, cwd))
  return researchSourceKeys(right, cwd).some((key) => leftKeys.has(key))
}

export function resolveResearchSourcePath(value, cwd = '') {
  const path = typeof value === 'string' ? value.trim() : ''
  if (!path) return ''
  const base = typeof cwd === 'string' ? cwd : ''
  const combined = path.startsWith('/') || !base
    ? path
...
function researchSourceKeys(source, cwd) {
  return [...new Set([
    typeof source?.key === 'string' ? source.key : '',
    canonicalResearchSourceKey(source),
    cwd ? canonicalResearchSourceKey(source, cwd) : '',
  ].filter(Boolean))]
}
...
function normalizeSource(source, existingSources) {
  ...
  const previousKey = cleanText(source.previousKey, 2000)
  const existing = existingSources.find((item) => item.key === key)
    || existingSources.find((item) => {
      return item.id === requestedId && previousKey && item.key === previousKey
    })
  const maxId = existingSources.reduce((max, item) => Math.max(max, item.id), 0)
  const requestedSource = existingSources.find((item) => item.id === requestedId)
  const requestedIdAvailable = Number.isInteger(requestedId)
    && requestedId > 0
    && (!requestedSource
      || requestedSource.key === key
      || requestedSource.key === previousKey)
  ...
  const replacesLocation = Boolean(existing
    && previousKey
  ...
  return {
    id,
    key,
    url: replacesLocation ? url : url || existing?.url || '',
    path: replacesLocation
      ? cleanText(source.path, 2000)
      : cleanText(source.path, 2000) || existing?.path || '',
...
function normalizeInvalidLinks(links) {
  if (!Array.isArray(links)) return []
  return links.slice(0, 10).flatMap((link) => {
    const id = Number(link?.id)
    const href = cleanText(link?.href, 2000)
    return Number.isInteger(id) && id > 0 && href ? [{ id, href }] : []
  })
}
