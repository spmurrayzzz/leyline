...
import { researchSourcesMatch } from './research-state.js'

const markdown = new MarkdownIt({ html: false, linkify: false })

export function auditResearchReportCitations(text, sources, cwd = '') {
  const ids = new Set()
  const invalidLinks = []
  let invalid = 0
  for (const link of numericMarkdownLinks(text)) {
    const source = (sources || []).find((item) => item.id === link.id)
    const target = /^https?:\/\//i.test(link.href)
      ? { url: link.href }
      : { path: normalizedPath(link.href) }
    if (!source
      || source.status === 'excluded'
      || !researchSourcesMatch(source, target, cwd)) {
      invalid++
      if (invalidLinks.length < 10) invalidLinks.push({ id: link.id, href: link.href })
      continue
    }
    ids.add(link.id)
  }
  return { ids: [...ids], invalid, invalidLinks }
}
